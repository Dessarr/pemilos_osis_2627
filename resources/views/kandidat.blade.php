...
.carousel-container-2 {
    position: relative;
    width: 100%;
    aspect-ratio: 1202 / 863; /* ⬅ biarkan CSS yang hitung tinggi */
    overflow: hidden;
    background: radial-gradient(ellipse, rgba(255, 224, 100, 0.92) 5%, #fef5d2 15%, #fef5d2 100%);
    border-radius: 16px 16px 0 0;
}

/* Semua gambar benar-benar fill */
.carousel-img {
    position: absolute;
    inset: 0;
    width: 100%;
...
.custom-scrollbar {
    ...
    scrollbar-color: rgba(255, 224, 100, 0.8) rgba(255, 224, 100, 0.2);
}

.custom-scrollbar::-webkit-scrollbar { width: 10px; }
.custom-scrollbar::-webkit-scrollbar-track {
    background: rgba(255, 224, 100, 0.15);
    border-radius: 10px;
    margin: 12px 4px;
}
.custom-scrollbar::-webkit-scrollbar-thumb {
    background: linear-gradient(180deg, rgba(255,224,100,.9), rgba(255,224,100,.6), rgba(255,224,100,.9));
    border-radius: 10px;
    border: 2px solid rgba(255,224,100,.3);
}
.custom-scrollbar::-webkit-scrollbar-thumb:hover {
    background: linear-gradient(180deg, rgba(255,224,100,1), rgba(255,224,100,.8), rgba(255,224,100,1));
}
</style>
    ...
    function initCarousel() {
        ...
        paslonContainers.forEach(function(container) {
            ...
            const intervalId = setInterval(function() {
                formalImg.style.opacity = isFormal ? '0' : '1';
                unformalImg.style.opacity = isFormal ? '1' : '0';
                isFormal = !isFormal;
            }, 5500);
...
/* === Carousel modal (fade only, NO height calc) === */
let modalCarousels = {};

function startModalCarousel(kandidatId) {
    ...
    images.forEach(img => img.classList.remove('carousel-img-active'));
    images[0].classList.add('carousel-img-active');

    let index = 0;
    if (modalCarousels[kandidatId]) {
        clearInterval(modalCarousels[kandidatId]);
    }

    modalCarousels[kandidatId] = setInterval(() => {
        images[index].classList.remove('carousel-img-active');
        index = (index + 1) % images.length;
        images[index].classList.add('carousel-img-active');
    }, 4000);
}

/* Livewire modal open hook */
document.addEventListener('livewire:init', function () {
    Livewire.on('modal-opened', function (data) {
        if (data && data.kandidatId) {
            setTimeout(() => startModalCarousel(data.kandidatId), 200);
        }
    });
});
</script>
